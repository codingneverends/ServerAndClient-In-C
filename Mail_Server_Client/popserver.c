...
int getslno(char *data){
    int flag=0;
    int slno=0;
    for(int i=0;i<strlen(data);i++){
        if(flag==1)
            slno=10*slno+(data[i]-'0');
        if(data[i]==' ')
            flag=1;
    }
    return slno;
...
void *handle_client(void *arg)
{
    ...
    while (1)
    {
        int deletionhappened=0;
        char mailbox_filename[100];
        sprintf(mailbox_filename, "%s/mymailbox.mail", client->username);
        FILE *mailbox = fopen(mailbox_filename, "r");
            
        char mailboxtemp_filename[100];
        sprintf(mailboxtemp_filename, "%s/mymailboxtemp.mail", client->username);
        FILE *mailboxtemp = fopen(mailboxtemp_filename, "w");
        ...
        if (strcmp(data, "GET_ALL") == 0)
        {
            ...
            while (mailbox && fscanf(mailbox, "%[^\n]%*c", line) != EOF)
            {
                ...
                if (attr == 3)
                {
                    count += sprintf(buffer + count, "%d | %s | %s | %s\n",slno, sender, time, subject);
                    attr = 0;
                    slno++;
                }
        ...
        if(strcmp(data,"VIEW")==0){
            char *intstring;
            int len = recv_packet(client->sock_fd, &intstring);
            int slno = atoi(intstring);
            ...
            if (count == 0)
            {
                count = sprintf(buffer, "Invalid Serial Number\n");
            }

            len = send_packet(client->sock_fd, buffer, count);
        }
        if(strncmp(data,"DELETE",6)==0){
            ...
            fprintf(mailboxtemp,"%s",buffer);
            strcpy(buffer,"Removed Sucessfully");
            deletionhappened=1;
            len = send_packet(client->sock_fd, buffer, sizeof(buffer));
        }
        ...
        fclose(mailbox);
        fclose(mailboxtemp);
        if(deletionhappened==1){
            //Do rename
            remove(mailbox_filename);
            rename(mailboxtemp_filename,mailbox_filename);
            deletionhappened=0;
        }
        remove(mailboxtemp_filename);
        free(data);
    }
